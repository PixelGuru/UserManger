...
const FormToDo = () => {
  ...
  const fetchData = () => {
    setItemloading(true);
    axios
      .get("https://642abcfb00dfa3b5474dab62.mockapi.io/product")
      .then((res) => {
        setJobs(res.data);
  ...
  const onAdd = () => {
    if (data.job.trim() === "") {
      ...
      setItemloading(true);
      axios
        .post("https://642abcfb00dfa3b5474dab62.mockapi.io/product", data)
        .then((res) => {
          setData({ job: "" });
  ...
  const onSaveEdit = () => {
    axios
      .put(
        `https://642abcfb00dfa3b5474dab62.mockapi.io/product/${editingJob.id}`,
        data
      )
      .then((res) => {
        setEditingJob(null);
  ...
  const onDelete = (id) => {
    Modal.confirm({
      ...
      onOk() {
        axios
          .delete(`https://642abcfb00dfa3b5474dab62.mockapi.io/product/${id}`)
          .then((res) => {
            fetchData();
          });
      },
